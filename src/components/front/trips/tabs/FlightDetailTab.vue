<template>
    <div class="flight-tab-wrapper">
        <template v-for="(item, i) in details.itinerary">
            ...
            <div v-for="(flight,flightIndex) in item.flightSegment" class="flight-tab-list" :key="'segment'+Math.random() + flightIndex">
                <div v-if="!flight.changePlane">
                   ...
                   <div>
                       <p>{{flight.planeDetails.airlineName}}{{flight.planeDetails.airlineFlNumber}} Opertated by:{{flight.planeDetails.operatedBy}} AirCraft Type:{{flight.planeDetails.aircraftType}} Terminal:{{flight.planeDetails.destTerminal}}</p>
                   </div>
                  <!--  <div class="col-sm-6">
